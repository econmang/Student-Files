...
{  
	...
	public void actionPerformed(ActionEvent arg) 
	{
			try
			{
				...
				{
					case "Add":
						DefaultMutableTreeNode newNode = new DefaultMutableTreeNode("Node " + childName++);
						DefaultTreeModel treeModel = new DefaultTreeModel(newNode);
						treeModel.insertNodeInto(newNode,selection,selection.getChildCount());
						tree.scrollPathToVisible(new TreePath(newNode.getPath()));
						break;
					case "Remove":
						if (selection.isLeaf() == false) {throw new Exception();}
						DefaultMutableTreeNode deleteNode;
					    treeModel = (DefaultTreeModel) (tree.getModel());
					    TreePath[] paths = tree.getSelectionPaths();
					    for (int i = 0; i < paths.length; i++)
					      {
					      deleteNode = (DefaultMutableTreeNode) (paths[i].getLastPathComponent());
						...
						break;
					case "Change":
						selection.setUserObject("Num " + childName++);
						treeModel = new DefaultTreeModel(selection);
			...
			{
				JOptionPane.showMessageDialog(this, "Must Be Leaf Node");
			}
	}
}