    ...
    def can_enroll(self,course_to_check):
        for course in self._enrolled_courses:
            if course.overlaps(course_to_check):
                print("Course times overlap")
                return False
        return True

    def dup_check(self,course_to_check):
        clone_courses = set(self._enrolled_courses)
        clone_courses.add(course_to_check)
        if (len(self._enrolled_courses) == len(clone_courses)):
            print("Cannot add duplicate courses")
            return False
        return True

    def enroll(self,course):
        if self._credits < 18 and (self._credits + course.num_credits) <= 18:
            if self.can_enroll(course) and self.dup_check(course):
