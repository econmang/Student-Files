...
{  
   ...
   public void run() {  
	   try {
		   ...
		   String str = "";
		   mt.start();
		   // loop over processes in the batch
		   ...
		   // at end of run, write my reference into my ProcessList
		   // so the kernel can determine who is finished
		   mt.interrupt();
		   writer.flush();
		   writer.close();
		   if (str == "")
		   ...
		   if(proc.className.equals("ReaderWriter")){
			   proc.classInstance.terminate();
		   }
		   while(!runList.isEmpty()){
				proc = runList.head();
